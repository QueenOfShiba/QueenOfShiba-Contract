...
contract QUEENSHIBADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("QUEENSHIBA_Dividend_Tracker", "QUEENSHIBA_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 10**13 * (10**9); //must hold 0.001% tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "QUEENSHIBA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "QUEENSHIBA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QUEENSHIBA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "QUEENSHIBA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "QUEENSHIBA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract QUEENSHIBA is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    QUEENSHIBADividendTracker public dividendTracker;
    ...
    constructor(address _routerAddress,address payable _marketingWallet ) ERC20("Queen of Shiba", "QUEENSHIBA") {
        BNBRewardsFee = 5;
        liquidityFee = 3;
    	...
    	dividendTracker = new QUEENSHIBADividendTracker();
        ...
        */
        //100,000,000,000,000,000 token
        _mint(owner(), 10**17 * (10**9));
        maxSellTransactionAmount = totalSupply().mul(2).div(1000);
        swapTokensAtAmount = maxSellTransactionAmount.div(10);
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "QUEENSHIBA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "QUEENSHIBA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "QUEENSHIBA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "QUEENSHIBA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "QUEENSHIBA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "QUEENSHIBA: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function setSwapAtAmount(uint256 _swapAtAmount) public onlyOwner {
        swapTokensAtAmount = _swapAtAmount;
    }
